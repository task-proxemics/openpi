"""Dual SO-101 robotic arms policy transforms for OpenPI with RGB cameras only."""
...
class S0101DualInputs(transforms.DataTransformFn):
    """
    Input transforms for dual SO-101 robotic arms with RGB cameras only.
    
    Two SO-101 arms with 6 degrees of freedom each:
    ...
    def __call__(self, data: dict) -> dict:
        ...
        if "observation/joint_positions" in data:
            ...
            all_joint_keys = left_joint_keys + right_joint_keys
            state = np.array([data.get(key, 0.0) for key in all_joint_keys])
        
        # Ensure we have 12 DOF
        if len(state) != 12:
            raise ValueError(f"Expected 12 joint positions for dual SO-101, got {len(state)}")
...
@dataclasses.dataclass(frozen=True)
class S0101DualOutputs(transforms.DataTransformFn):
    """Output transforms for dual SO-101 robotic arms."""
    
    def __call__(self, actions: np.ndarray) -> dict:
        """
        Convert model actions to dual SO-101 joint commands.
        
        Args:
            actions: Model actions (12,) array [left_arm(6) + right_arm(6)]
            
        Returns:
            Dictionary with joint commands for dual SO-101
        """
        # Ensure we have 12 actions for dual SO-101
        if actions.shape[-1] != 12:
            raise ValueError(f"Expected 12 actions for dual SO-101, got {actions.shape[-1]}")
        
        # Split actions into left and right arms
        left_actions = actions[..., :6]
        right_actions = actions[..., 6:]
